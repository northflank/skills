...
You can also manage your account and projects through the Northflank [API, CLI, and JavaScript client](migrate-from-heroku.md#use-the-northflank-api), helping you develop your applications locally using forwarding and command execute, and programmatically manage your entire devops workflow.
...
4. Build options: choose [buildpack](build.md#build-code-from-a-git-repository-build-with-buildpacks) to automatically build your application. Northflank uses the Heroku 20 stack by default. You can select another buildpack stack, or add custom buildpacks, in advanced build settings.

5. Environment variables (optional): you can set [build arguments](build.md#build-code-from-a-git-repository-specify-build-arguments) and [runtime variables](secure.md#inject-secrets-runtime-variables), or add [secret files](secure.md#upload-secret-files) in advanced. You can add your connection details for hosted databases or API keys for other services here to make them available within your application.
...
You can learn more about [managing combined services](getting-started.md#build-and-deploy-your-code-create-a-combined-service) in our getting started guide, or check the documentation for specific tasks you want to achieve.
...
1. [Add a domain](domains.md#add-a-domain-to-your-account).

2. [Add a subdomain](domains.md#add-a-domain-to-your-account-add-a-subdomain).

3. [Link a domain to a port](domains.md#link-a-domain-to-a-port).
...
To get started on Northflank you can use your existing databases and other service integrations by including their API keys, connection strings, or other secrets as environment variables. Read more about [secret management](secure.md#inject-secrets).

Northflank offers [several managed databases](databases-and-persistence.md#deploy-a-database), including [PostgreSQL](databases-and-persistence.md#deploy-postgresql-on-northflank), [Redis](databases-and-persistence.md#deploy-redis®-on-northflank), [MongoDB](databases-and-persistence.md#deploy-mongodb®-on-northflank), [MySQL](databases-and-persistence.md#deploy-mysql-on-northflank), and [MinIO](databases-and-persistence.md#deploy-minio®-on-northflank), that can be deployed with a click.

Select addon from the create new and choose a database to deploy.

After deploying a database you can import your data from the [backups page](databases-and-persistence.md#backup-restore-and-import-data) by uploading a text dump or specifying the connection string of the database you wish to import from. You have several options to automatically or manually manage [runtime migrations](https://northflank.com/docs/v1/application/release/handle-runtime-migrations) on Northflank.

You can also create [persistent volumes](databases-and-persistence.md#add-a-persistent-volume) to attach to your deployments, for any use cases not covered by our managed addons.
...
If you're using Heroku scheduler you can create [cron jobs](run.md#run-an-image-once-or-on-a-schedule) on Northflank that will run your applications at specific times, before shutting down.
...
To get started, create an [API token](secure.md#grant-api-access-generate-an-api-token), or create an [API role](secure.md#grant-api-access) for your team to use if you don't have one available.
...
- [Use the Northflank JavaScript client: Learn how to create and manage projects on Northflank programmatically using the JavaScript client.](../api/use-the-javascript-client.md)
...
You can create a build service linked to the Git repository for your application, and build commits to specific branches and pull request branches by adding [build rules](build.md#build-code-from-a-git-repository-build-specific-branches-or-pull-requests).
...
3. Repository: select the Git repository from the drop-down list and choose the branch you want to build from. You can also set [build rules](build.md#build-code-from-a-git-repository-build-specific-branches-or-pull-requests) to build specific branches or pull request branches.

4. Build options: choose [buildpack](build.md#build-code-from-a-git-repository-build-with-buildpacks) to automatically build your application. Northflank uses the Heroku 20 stack by default. You can select another buildpack stack, or add custom buildpacks, in advanced build settings. You can also set the [build context](build.md#build-code-from-a-git-repository-build-a-specific-repository-directory), if it's not the root directory.

5. Environment variables (optional): you can set [build arguments](build.md#build-code-from-a-git-repository-specify-build-arguments) or add [secret files](secure.md#upload-secret-files) in advanced

6. Resources: choose a plan with more CPU and memory available to your builds, if required

[Learn more about building on Northflank](build.md#build-code-from-a-git-repository).
...
You can either add variables required at runtime directly in the deployment services on the environment page, or create a [secret group](secure.md#manage-secret-groups) that can be inherited by multiple services.
...
4. Environment variables (optional): you can set [runtime variables](secure.md#inject-secrets-runtime-variables) or add [secret files](secure.md#upload-secret-files) in advanced
...
[Learn more about deploying on Northflank](run.md#run-containers-and-micro-services-on-northflank).
...
[Learn more about secrets on Northflank](secure.md#security-on-northflank).

### Import a Heroku pipeline to Northflank

To import your pipeline to Northflank manually you can create a build service for each repository you wish to build from, and set [rules](build.md#build-code-from-a-git-repository-build-specific-branches-or-pull-requests) to build specific branches or pull requests.
...
You can follow the steps in [import a Heroku application with multiple dynos to Northflank](migrate-from-heroku.md#import-a-heroku-application-with-multiple-dynos-to-northflank) to create build services, deployment services, and secret groups.
